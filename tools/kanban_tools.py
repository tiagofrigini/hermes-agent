...
def _resolve_task_scoped_board(board: Optional[str]) -> Optional[str]:
    """Resolve an explicit board without letting task-scoped calls escape it."""
    if board is None:
        return None
    ...
    env_board = kb._normalize_board_slug(os.environ.get("HERMES_KANBAN_BOARD"))
    if env_board is None:
        raise ValueError(
            "HERMES_KANBAN_BOARD is required when board is explicit in a "
            "task-scoped worker"
        )
    if board_slug != env_board:
        raise ValueError(
            f"board {board_slug!r} does not match task board {env_board!r}"
        )
    return board_slug
...
def _handle_create(args: dict, **kw) -> str:
    ...
    if (
        ...
        return tool_error("max_retries must be a positive integer")
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "max_retries": {
                ...
                "minimum": 1,
                "description": (
                    "Per-task consecutive-failure circuit-breaker limit. "
                    "For example, 1 blocks after the first failed run; omit "
                    "to use the board-level kanban.failure_limit setting."
                ),
            },
            "initial_status": {
                "type": "string",
