...
# Keep exact names so user-authored HERMES_KANBAN_* settings still persist.
...
def _export_dump_excluding_session_vars(
    ...
    return (
        "{ ( "
        f"unset {_SNAPSHOT_EXCLUDED_UNSET_NAMES} "
...
class BaseEnvironment(ABC):
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        if self._snapshot_ready:
            # Preserve the invocation-scoped delegated marker across sourcing so
            # stale snapshots cannot invent lineage and genuine children retain it.
